...
struct BatchStats {
    ...
    double pq_score_kernel_us = 0.0;
    double d2h_us = 0.0;
    double cpu_rerank_us = 0.0;
...
static void compute_rerank_recall_from_pairs_cpu(
    ...
    std::vector<std::priority_queue<Pair, std::vector<Pair>, std::greater<Pair>>> approx_heaps(
    ...
    for (size_t i = 0; i < pair_count; ++i) {
        ...
        float pq_score = pair_scores[i];

        auto& heap = approx_heaps[qid];

        Pair item{pq_score, bid};

        if (heap.size() < rerank_r) {
            heap.push(item);
    ...
    for (int b = 0; b < current_batch_size; ++b) {
        const float* q =
            query + (query_global_start + static_cast<size_t>(b)) * vecdim;

        std::priority_queue<Pair, std::vector<Pair>, std::greater<Pair>> exact_heap;

        auto& approx_heap = approx_heaps[static_cast<size_t>(b)];

        while (!approx_heap.empty()) {
            uint32_t bid = approx_heap.top().second;
            approx_heap.pop();

            const float* x = base + static_cast<size_t>(bid) * vecdim;
            float exact_score = inner_product_cpu(q, x, vecdim);

            Pair item{exact_score, bid};

            if (exact_heap.size() < k) {
                exact_heap.push(item);
            } else if (item.first > exact_heap.top().first) {
                exact_heap.pop();
                exact_heap.push(item);
            }
        }
        ...
        while (!exact_heap.empty()) {
            uint32_t id = exact_heap.top().second;
...
int main(int argc, char* argv[]) {
    /*
        Usage:
            main_gpu_ivfpq_rerank.exe <data_path> <test_number> <batch_size>
                                      <nlist> <nprobe> <ivf_iter>
                                      <M> <Ks> <pq_train_samples> <pq_iter>
                                      <rerank_r> <k>

        Example:
            main_gpu_ivfpq_rerank.exe ./data/ 1000 16 1024 50 10 16 256 8000 6 100 10
    */
    ...
    size_t rerank_r =
        argc >= 12 ? static_cast<size_t>(std::stoul(argv[11])) : 100;

    size_t k =
        argc >= 13 ? static_cast<size_t>(std::stoul(argv[12])) : 10;
    ...
    while (processed < test_number) {
        ...
        auto rerank_begin = std::chrono::steady_clock::now();

        compute_rerank_recall_from_pairs_cpu(
            h_pair_scores,
            pair_base_ids,
            pair_query_ids,
            current_batch_size,
        ...
        total_stats.pq_score_kernel_us += score_ms * 1000.0;
        total_stats.d2h_us += d2h_ms * 1000.0;
        total_stats.cpu_rerank_us += elapsed_us(rerank_begin, rerank_end);
    ...
    double avg_pq_score_kernel_per_query_us =
        total_stats.pq_score_kernel_us / query_count_d;

    double avg_d2h_per_query_us =
        total_stats.d2h_us / query_count_d;

    double avg_cpu_rerank_per_query_us =
        total_stats.cpu_rerank_us / query_count_d;
    ...
    std::cout << "average PQ score kernel per query (us): "
              << avg_pq_score_kernel_per_query_us << "\n";

    std::cout << "average D2H per query (us): "
              << avg_d2h_per_query_us << "\n";

    std::cout << "average CPU rerank per query (us): "
              << avg_cpu_rerank_per_query_us << "\n";
